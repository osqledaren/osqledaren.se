...
function osqledaren_next_post() {
  ...
  if (!empty($next_post)):
    ...
    <a href="<?php echo get_permalink( $next_post->ID ) ?>"><div class="next"<?php echo $background; ?> >
      <div class="overlay">
       <div class="row">
        <div class="padding">
         <div class="meta clearfix">
          <?php if (is_singular('podcast')): ?>
          <p class="desc">Nästa podd:</p>
          <div class="time"><?php echo get_post_custom_values('duration', $next_post->ID)[0]; ?></div>
        <?php else: ?>
...
function the_archive_description($before = '', $after = '') {
  ...
  if (!function_exists('the_podcast_series')):

  /**
   * Renders a list of podcast series which the podcast belongs to.
   * @return string
   */
function the_podcast_series() {
  global $post;

  $termsobj = get_the_terms($post->ID, 'series');
  $terms    = array();
